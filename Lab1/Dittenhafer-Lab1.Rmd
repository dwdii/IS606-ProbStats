...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    * exerany; numeric discrete (binary) - could be considered categorical, nominal in the concept of true/false
    * hlthplan: numeric discrete (binary) - could be considered categorical, nominal in the concept of true/false
    * smoke100: numeric discrete (binary) - could be considered categorical, nominal in the concept of true/false
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    The bmi by genhlth boxplot shows that those who report excellent general health tend to have the lowest
    BMI, and also the narrowest interquartile range. As general health decreases, BMI rise and the range increases.
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    ```{r}
    # Visualize and include a line representing 0 diff (wtdesire = weight)
    wdw <- ggplot(data=cdc) + 
      geom_point(aes(x=weight, y=wtdesire), alpha = 2/10, colour="darkblue") + 
      geom_abline(intercept=0, slope=1, colour="lightblue") + 
      myTheme
    wdw
    # Compare desired weight to weigth and show more people desire 
    # a lower weight than they currently report.
    dwltw <- cdc$wtdesire < cdc$weight
    tblDwltd <- table(dwltw)
    barplot(tblDwltd)
    title(main="Desire lower weigth than current?")
    ```
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    wdiff is numeric, discrete. If an observation is 0 then this means the person
    is at their desired weigth. If wdiff is positive, then the person is above their
    desired weight, otherwise they are below their desired weight.
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    tblWdiffSign <- table(cdc$wdiffsign, cdc$gender)
    tblWdiffSign
    tblWdiffSign[,"m"] <- tblWdiffSign[,"m"] / sum(tblWdiffSign[,"m"])
    tblWdiffSign[,"f"] <- tblWdiffSign[,"f"] / sum(tblWdiffSign[,"f"])
