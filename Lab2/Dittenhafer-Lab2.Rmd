...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    **Streak length of 1 means 1 basket followed by a miss. There is 1 hit and 1 miss in a streak of 1. A streak of 0 is only a miss.**
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    **The distribution is skewed to the right and is unimodal. The typical streak is 0 and his longest streak is 4.**
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    **In my simulation of flipping the unfair coin, 18 flips came up heads.**
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    **The distribution is skewed right and unimodal. Typical streak length is 0. Longest streak is `r max(sim_streak)`.**
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    **I would expect a similar distribution, though not exactly the same. The probabilities are the same, so it is still more likely to miss than hit, but the shape of the distribtion should still hold with a right skew.**
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    **Kobe's distribution is very similar, with 0 streak being the more prevelant. I would not say I have evidence that the hot hand model fits or does not fit Kobe's shooting patterns, but the results are suggestive of Kobe's shots being independent events rather than dependent hot hand events. Interestingly, when running a 60/40 hit/miss probability, the distribution is not much different, though the max spread does tend to increase. See barplot below: **
