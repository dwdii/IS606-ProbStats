...
1.  Describe this population distribution.
    ...
    **The population distribution appears fairly normal, unimodal and slightly skewed to the right. As shown in QQ plot below, the central points lie nicely on the line, but the tails deviate.**
...
2.  Describe the distribution of this sample. How does it compare to the distribution of the population?
    ...
    **As shown in the histogram below, we see the sample distribution is bimodal and skewed slightly right. The QQ plot also shows a much less normal distribution even on the central points as compared to the population distribution.**
    ```{r}
    hist(samp1)
    qqnorm(samp1)
    qqline(samp1)
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    mnSamp2 <- mean(samp2)
    dfMeans <- data.frame(Mean1=c(mnSamp1), Mean2=c(mnSamp2))
    dfMeans
    ```
    
    **The larger the sample, the closer we will get to the population mean. The sample of size 1000 would be preferred over the 100 or the 50.**
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    **`sample_means50` contains 5000 elements. The sampling distribution is very much in a normal shape, unimodal with a center just shy of 1500. As shown in the QQ plot, the sampling distribution is very close to the normal line. If we collected 50,000 sample means, by the Central Limit Theorem, the distribution would even more closely match the normal distribution and center around the population mean.**
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    **There are 100 elements in the `sample_means_small` object and each element represents the mean of a sample from the area population.**
...
**When the sample size is larger, the center become more prominent and the spread narrows. **
...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    ```{r}
    sPrice50 <- sample(price, 50)
    mPrice50 <- mean(sPrice50)
    mPrice50
    ```
...
-   Since you have access to the population, simulate the sampling 
    ...
    hist(sample_means50, 32)
    ```
    
    
    **The sampling distribution has a bell shape and looks fairlly normal. It is unimodal with only a slight right skew.**
    
    **I would estimate the mean home price of the population to be 176000-178000.**
    
    ```{r}
    popMeanPrice <- mean(price)
    popMeanPrice
    ```
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    **As shown below, the shape of the distribution is unimodal, bell with very little skew. This sample distribution has a more uniform, narrower spread also.**
    ...
    hist(sample_means150, 32)
    ```
    
        **I would estimate the mean home price of the population to be 182000-186000.**
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    **The sampling distribution from the 150 sample (#3) has a smaller spread. As can be seen below, the standard deviation of the sample distribution for the 50 set is much higher than the 150 set.**
    
    ```{r}
    sd50 <- sd(sample_means50)
    sd150 <- sd(sample_means150)
    dfSd <- data.frame(sd50, sd150)
    dfSd
    ```
