...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    ```{r}
    g1 <- ggplot(data=mdims) + geom_bar(aes(x=hgt)) +
            myTheme +
            labs(title="Men's Height Distribution", x="Height (cm)")
    g1
  
    g2 <- ggplot(data=fdims) + geom_bar(aes(x=hgt)) +
            myTheme +
            labs(title="Women's Height Distribution", x="Height (cm)")
    g2
    
    g3m <- ggplot() + 
            geom_bar(data=mdims, aes(x=(hgt %% 2) == 0), colour="lightblue", position="dodge") +
            myTheme +
            labs(title="Even vs Odd Heights", x="")
    g3m
    
    g3f <- ggplot() + 
            geom_bar(data=fdims, aes(x=(hgt %% 2) == 0), colour="pink", position=position_dodge(width=1)) +
            myTheme +
            labs(title="Even vs Odd Heights", x="")
    g3f
