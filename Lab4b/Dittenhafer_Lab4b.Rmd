...
**My sample distribution is bimodal, or possibly multimodal. Based strictly on the visual distribution, I would say the typical size is around 1000 because the 2 main peaks are on either side of 1000.**

```{r, message=FALSE}
dfSamp <- data.frame(Gr.Liv.Area=samp)
g1 <- ggplot(data=dfSamp) + geom_bar(aes(x=Gr.Liv.Area)) + myTheme
g1
```
...
**I would not expect another student's distributin to be identical, but I would expect some similarlity. Any given sample could select a wide range of values from the population, making any two samples extremely unlikely to be identical. With that said, each sample is drawn from the same population and will have some similarity to the population and transitively a similarity to other samples from the same population.**
...
**I would expect 95% of the intervals to capture the true population mean as this is the definition of the 95% confidence interval.**
