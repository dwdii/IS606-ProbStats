...
# Column names
names(present)
# Show statistics about the year values
summary(present$year)
```

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?

As shown below, Arbuthnot's data contained much fewer births. Present data is several
magnitudes greater.

```{r}
# Load Arbuthnot's data and show summary along with present data summary for comparison
source("more/arbuthnot.R")
summary(arbuthnot)
# Show summary of present data
summary(present)
```
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    As shown below, the data supports Arbuthnot's observation about boys being born
    in greater proportion than girls and interestingly, the range is narrower 
    (between 51 - 52%).
...
plot(present$year, 
     ...
     type="l", 
     main="Ratio of boys born per year")
```
...
```{r}
# Aggregate to totals and update col names
totals <- data.frame(present$year, present$boys + present$girls)
names(totals) <- c("year", "totalBirths")
# Sort by births descending
totals <- totals[order(totals$totalBirths,decreasing = TRUE),]
